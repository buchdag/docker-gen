...
dist: dist-clean
	...
	mkdir -p dist/alpine-linux/i386 && GOOS=linux GOARCH=386 go build -ldflags "$(LDFLAGS)" -a -tags netgo -installsuffix netgo -o dist/alpine-linux/i386/docker-gen ./cmd/docker-gen
...
release: dist
	...
	tar -cvzf docker-gen-alpine-linux-i386-$(TAG).tar.gz -C dist/alpine-linux/i386 docker-gen
